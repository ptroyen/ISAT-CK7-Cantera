...
!
! info(65) = mpi_log   = 0, for log file for each processor; 
!                      = 1, for log file for processor 0 only
! info(66) = mpi_uniq  = 0, use unique file names (isat_n_p.tail)
!                      = 1, use the same file name on each processor (isat_n.tail)

! VH, 11/28/2012 - set mpi_nml = 1 to use single nml file for all the cores 
! info(67) = mpi_nml   = 0, use unique input nml files (isat_n_p.nml)
!                      = 1, use a single nml file for all the cores (isat_n.nml)
!
...
! 
! VH, 11/28/2012 - info(72) can be used to save only one table per partition
!     (this option is currently not being used, we save/read all the tables)
! info(72) = 0, save/read ISAT tables with name isat_n_p.tab
!          > 0, save/read ISAT tables with name isat_n_info(72).tab
!
